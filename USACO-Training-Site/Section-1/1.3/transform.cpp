...
bool isSame(char *arr, char *arr2, int n) {
    for(int i=0; i<n; i++) {
        for(int j=0; j<n; j++) {
            if(*((arr+i*n)+j)!=*((arr2+i*n)+j)) return false;
        }
    }
    return true;
}
...
void solve() {
    ...
    f0r(i, n) {
        f0r(j, n) {
            cin >> arr2[i][j];
            if(arr[i][j]!=arr2[i][j]) flag = false;
        }
    }
    int cnt = -1;
    for(int i=1; i<=4; i++) {
        ...
        for(int c=0, j=0; c<n; c++, j++) {
            for(int r=n-1, k=0; r>=0; r--, k++) {
                tmp2[j][k] = tmp[r][c];
            }
        }
        if(isSame((char *)tmp2, (char *)arr2, n)) {
            cout << i << "\n";

            return;
        }
        f0r(j, n) f0r(k, n) tmp[j][k] = tmp2[j][k];
    }
    if(cnt!=-1) {
        cout << cnt << "\n";
        return;
    ...
    if(isSame((char *)tmp, (char *)arr2, n)) {
        cout << "4\n";
        return;
    }
    for(int i=1; i<=4; i++) {
